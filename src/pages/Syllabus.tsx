...
// =====================================================================
// API base — reads VITE_API_BASE from .env
// =====================================================================
...
// =====================================================================
// Chapter Summary cache — localStorage so we never regenerate
// =====================================================================
const summaryKey = (chapterId: string) => `syllab:summary:${chapterId}`;

function loadCachedSummary(chapterId: string): string | null {
  try {
    return localStorage.getItem(summaryKey(chapterId));
  } catch {
    return null;
  }
}

function saveCachedSummary(chapterId: string, summary: string) {
  try {
    localStorage.setItem(summaryKey(chapterId), summary);
  } catch { /* quota */ }
}

async function fetchChapterSummary(chapter: Chapter): Promise<string> {
  const res = await fetch(`${API_BASE}/api/chapter-summary`, {
    method: 'POST',
...
// =====================================================================
// ChapterCard — handles its own AI summary fetching with caching
// =====================================================================
function ChapterCard({
  ...
  const [summary, setSummary] = useState<string | null>(() => loadCachedSummary(chapter.id));
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(false);

  useEffect(() => {
    if (summary) return; // already cached
    let cancelled = false;
    setLoading(true);
    setError(false);

    fetchChapterSummary(chapter)
      .then((s) => {
        if (cancelled) return;
        if (s) {
          setSummary(s);
          saveCachedSummary(chapter.id, s);
        } else {
          setError(true);
        }
      })
        ...
        <div className="flex flex-wrap gap-2">
          <button 
            ...
            className={cn(
              ...
              isPinned ? "text-primary bg-primary/10" : "text-slate-300 hover:text-slate-400"
            )}
          >
        ...
        <div className="space-y-2">
          ...
          {loading && (
            <div className="space-y-2 animate-pulse">
              ...
              <div className="h-3 bg-slate-100 rounded-full w-9/12" />
          ...
          {!loading && !summary && error && chapter.explanation && (
            <p className="text-sm text-slate-500 font-medium leading-relaxed">
              {chapter.explanation}
            </p>
          )}
...
export default function SyllabusPage({ setTab, syllabus, setPracticeConfig }: SyllabusPageProps) {
  ...
  // Read ?class=N from URL or sessionStorage on first load
  // This makes home page Class N buttons land on the right filtered view.
  useEffect(() => {
    let target: string | null = searchParams.get('class');
    if (!target) {
      try { target = sessionStorage.getItem('syllab:pending_class'); } catch { /* ignore */ }
    }
    if (target) {
      const validClasses = ['5', '6', '7', '8', '9', '10', '11', '12'];
      if (validClasses.includes(target)) {
        setSelectedClass(target as ClassLevel);
      }
      try { sessionStorage.removeItem('syllab:pending_class'); } catch { /* ignore */ }
    }
  }, [searchParams]);
  ...
  // ORIGINAL class list — Class 5 to 12 only (matches your existing syllabus data)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredChapters.map((chapter) => (
          <ChapterCard
            key={chapter.id}
            chapter={chapter}
            isPinned={pinnedChapters.includes(chapter.id)}
            onPin={() => togglePin(chapter.id)}
            onLearn={() => void handleConcept(chapter)}
            onAITutor={() => setTab('tutor')}
            onPractice={() => handlePractice(chapter)}
          />
        ))}
      </div>
