...
// IMPORTANT: .env should contain:
// VITE_API_URL=https://syllab.onrender.com
// =====================================================================
const API_BASE: string = (() => {
  if (typeof window !== 'undefined' && (window as any).__SYLLAB_API__) {
    return String((window as any).__SYLLAB_API__).replace(/\/+$/, '');
  }

  if (typeof window !== 'undefined' && (window as any).__PADHAI_API__) {
    return String((window as any).__PADHAI_API__).replace(/\/+$/, '');
  }
  ...
  // Production-safe fallback. This prevents syllab.in from calling localhost.
  return 'https://syllab.onrender.com';
})();

console.log('[Arena] API_BASE:', API_BASE);
...
function toBankDifficulty(value: unknown): Diff {
...
function normalizeAIQuestion(
  ...
  return {
    ...
    id:
      raw?.id ||
      `${meta.chapterId}-${String(meta.difficulty)}-${Date.now()}-${meta.index}`,
    classLevel: raw?.classLevel || meta.classLevel,
    subject: raw?.subject || meta.subject,
    chapter_id: raw?.chapter_id || raw?.chapterId || meta.chapterId,
    chapterId: raw?.chapterId || raw?.chapter_id || meta.chapterId,
    chapterTitle: raw?.chapterTitle || meta.chapterTitle,
    difficulty: toBankDifficulty(raw?.difficulty || meta.difficulty),
    question_text: String(questionText),
    question: String(raw?.question || questionText),
    options,
...
async function fetchAIBatch(
  ...
  const results = await Promise.all(
    chunks.map(async (size, chunkIndex) => {
      try {
        const res = await fetch(`${API_BASE}/api/questions/batch`, {
          ...
          body: JSON.stringify({
            ...
            // Backend expects chapterTitle.
            chapterTitle,

            // Keep old field too for backward compatibility.
        ...
        if (!res.ok) {
          console.error('[questions/batch] HTTP error:', res.status, rawText);
          throw new Error(`HTTP ${res.status}: ${rawText || 'questions batch failed'}`);
        }
        ...
        try {
          ...
          console.error('[questions/batch] Invalid JSON:', rawText);
        ...
        return questions.map((q: any, i: number) =>
          normalizeAIQuestion(q, {
            classLevel,
            subject,
            chapterTitle,
            chapterId,
            difficulty,
            index: chunkIndex * CHUNK + i,
          }),
        );
      } catch (err) {
        done++;
...
export default function ArenaPage({
  ...
  useEffect(() => {
    const submitSession = async () => {
      ...
      const completedChapters = Array.from(
        new Set(quizQuestions.map((q) => q.chapter_id).filter(Boolean)),
      );
  ...
  const buildSession = async (
    ...
    fetched.forEach((q) => {
      const d = toBankDifficulty(q.difficulty);
      const bucket = updated[d];

      if (bucket.length < TARGET_PER_DIFF) {
        bucket.push(q);
      }
    });
  ...
  const startQuiz = async (overrideCount?: number, overrideDifficulty?: Difficulty) => {
    ...
    try {
      ...
      if (session.length === 0) {
        throw new Error('Could not generate any questions. Check backend.');
      }
        ...
        <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-slate-500">
                You have a saved practice session at question {pausedSession.currentIndex + 1}.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-6 py-2 rounded-xl text-slate-400 font-bold hover:bg-slate-100 transition-all text-sm"
            >
              Discard
            </button>
            ...
            <button
              ...
              className="flex items-center gap-2 px-8 py-3 bg-secondary text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-slate-900/20 hover:scale-105 transition-all"
            >
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
          <section className="relative mb-12 overflow-hidden rounded-[3rem] bg-gradient-to-br from-primary via-primary-light to-primary p-12 text-white shadow-2xl md:p-20">
            <div className="absolute inset-0 z-0 opacity-10">
              <img
                src="https://picsum.photos/seed/tactical/1200/600?blur=4"
                  ...
                  {availableClasses.map((value) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selClass === value
                          ? 'bg-primary text-white shadow-xl shadow-violet-500/20'
                    ...
                    <optgroup label="Pinned Chapters">
                      {SYLLABUS.filter((c) => pinnedChapters.includes(c.id)).map((c) => (
                        <option key={`p-${c.id}`} value={c.id}>
                          {c.title} ({c.subject} Class {c.classLevel})
                        </option>
                      ))}
                    </optgroup>
                  )}
                    ...
                    {['10', '20', '30', '40', '50'].map((c) => (
                      <button
                        ...
                        className={cn(
                          ...
                          selCount === c
                            ? 'border-2 border-orange-200 bg-orange-50 text-orange-600'
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2 rounded-2xl bg-slate-50 p-2">
                  <button
                    ...
                    className={cn(
                      'flex-1 rounded-xl py-1.5 text-[9px] font-black uppercase tracking-wider transition-all',
                      isTimerEnabled ? 'bg-primary text-white' : 'text-slate-400',
                    )}
                  >
                    Enabled
                  </button>
          ...
          <div className="mb-8 flex items-center justify-between">
            <div className="flex gap-4">
              ...
              <button
                ...
                className="flex items-center gap-2 text-sm font-black text-primary transition-all hover:gap-3 group"
              >
            ...
            <div className="flex items-center gap-6">
              {isTimerEnabled && (
                <div
                  className={cn(
                    'flex items-center gap-2 rounded-xl border-2 px-4 py-2 font-mono text-lg font-black',
                    timeLeft < 15
                      ? 'animate-pulse border-rose-100 bg-rose-50 text-rose-500'
                      : 'border-slate-100 bg-white text-primary',
                  )}
                >
              ...
              <div className="flex items-center gap-4">
                <div className="h-3 w-48 overflow-hidden rounded-full border border-slate-100 bg-slate-100 p-0.5">
                  <motion.div
                    className="h-full bg-primary rounded-full shadow-[0_0_10px_rgba(16,185,129,0.4)]"
                    initial={{ width: 0 }}
                  ...
                  {quizQuestions[currentIndex].options.map((option, index) => (
                    ...
                    >
                      <div className="flex items-center gap-5 relative z-10">
                        <div
                          className={cn(
                            'flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-xs font-black transition-all',
                      ...
                      <div className="relative z-10">
                        {isAnswered && index === quizQuestions[currentIndex].correct_index ? (
                          <CheckCircle2 className="text-emerald-500" size={28} fill="white" />
                        ) : null}

                        {isAnswered && selectedAnswer === index && index !== quizQuestions[currentIndex].correct_index ? (
                          <XCircle className="text-rose-500" size={28} fill="white" />
                        ) : null}
                      </div>
                    </button>
                  ))}
        ...
        >
          ...
          <div className="flex flex-col gap-4 sm:flex-row">
            <button
              type="button"
              onClick={() => setMode('config')}
              className="btn-primary flex-1 rounded-2xl py-5 text-lg"
            >
              Start New Session
            </button>
            ...
            <button
              ...
              onClick={() => {
                setMode('config');
                setSelChapter(quizQuestions[0]?.chapter_id || '');
              }}
          ...
          {!currentUser ? (
            <p className="mt-6 text-sm font-semibold text-slate-500">
              Sign in to store your results, completed chapters, and mistakes in Firestore.
            </p>
          ) : null}
        </motion.div>
      ) : null}
