  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <SEO
        ...
        description="Make a free, personalized study timetable in seconds. Enter your exam date, subjects and study hours — get a day-by-day revision plan (learn → revise → mock test) you can save or print. Built for CBSE, JEE & NEET students."
      ...
      />
      <PageHero emoji="🗓️" title="Study Planner" subtitle="Enter your exam date, subjects & daily hours — get a day-by-day revision timetable you can save or print. Free, no signup." className="mb-8" />
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-5 shadow-sm print:hidden">
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block">
            ...
            <input value={subjectsRaw} onChange={(e) => setSubjectsRaw(e.target.value)} placeholder="Maths, Physics, Chemistry, Biology"
        ...
        {plan.length > 0 && (
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <span className="text-xs font-bold text-slate-400">{plan.length} days · {subjects.length} subjects · ~{totalHours} study hours</span>
          </div>
        )}
      </div>
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-100 dark:border-slate-700">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 dark:bg-slate-800 text-[10px] font-black uppercase tracking-wide text-slate-400">
                <tr><th className="px-3 py-2">Day</th><th className="px-3 py-2">Date</th><th className="px-3 py-2">Focus</th><th className="px-3 py-2">Phase</th><th className="px-3 py-2">Hours</th></tr>
              </thead>
              <tbody>
                {plan.map((d, i) => (
