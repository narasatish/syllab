...
export const dataAnalyticsTopics: TutorialTopic[] = [
  // ── What is Data ─────────────────────────────────────────────────────────────
  {
    ...
    difficulty: 'Beginner',
    theory: [
      `Data Analytics is the science of examining raw data to draw conclusions about information. It involves collecting, processing, cleaning, and analysing data to discover useful insights, suggest conclusions, and support decision-making.`,
      `Every organisation — from a school tracking student marks to Swiggy tracking delivery times, to ISRO tracking satellite orbits — uses data analytics to make better decisions.`,
      `The 4 types of analytics: Descriptive (what happened?), Diagnostic (why did it happen?), Predictive (what will happen?), Prescriptive (what should we do?). Most school-level data work is Descriptive and Diagnostic.`,
      `Python, SQL, and Excel are the three main tools for data analytics. Python (with Pandas and NumPy) is the most powerful. Excel is the easiest to start with. SQL is essential for database data.`,
    ],
