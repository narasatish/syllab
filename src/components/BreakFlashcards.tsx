    ...
    <div className="mt-5 w-full">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        {cards.map((c, i) => (
          ...
          >
            {flipped[i] ? (
              <span><span className="text-[11px] font-black uppercase text-emerald-300">Answer</span><br />{c.a}</span>
            ) : (
              <span><span className="text-[11px] font-black uppercase text-amber-200/70">Q{i + 1}</span><br />{c.q}</span>
            )}
          </motion.button>
        ))}
      </div>
